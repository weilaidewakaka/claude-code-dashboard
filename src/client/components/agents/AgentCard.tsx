import type { AgentInfo, TokenLevel } from "../../../shared/types";
  ...
  return (
    <Card
      className={`group hover:ring-[var(--border-accent)] ${
        agent.parentPluginEnabled ? "" : "opacity-50"
      }`}
    >
      <CardContent className="p-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2.5">
            ...
            <span
              className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-medium tracking-wide ring-inset ${
                SOURCE_BADGE_VARIANT[agent.source]
              }`}
            >
              {agent.source === "plugin" ? agent.pluginName ?? "plugin" : agent.source}
            </span>
            {agent.source === "plugin" && agent.pluginId && (
              <span className="inline-flex items-center rounded-full bg-sky-500/10 px-2 py-0.5 text-[10px] font-medium text-sky-400 ring-1 ring-sky-500/20">
                {agent.pluginId.includes("@") ? agent.pluginId.slice(agent.pluginId.indexOf("@") + 1) : agent.pluginId}
              </span>
            )}
          </div>
          ...
          <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
            <Badge
              label={`${TOKEN_LABELS[agent.tokenLevel]} (${formatTokens(agent.estimatedTokens)})`}
              variant={agent.tokenLevel}
            />
            {!agent.parentPluginEnabled && (
